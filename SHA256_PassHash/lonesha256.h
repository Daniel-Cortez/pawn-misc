/*
Changes (by Stanislav Gromov):
	* Commented out "#include <stdint.h>"; amx.h already provides uint32_t and uint64_t types.
	* Added '#undef's at the end of file so the internal macros won't pollute the global namespace.
*/
...
LSHA256DEF int lonesha256(unsigned char*, const unsigned char*, unsigned long);
...
//lonesha256 function
LSHA256DEF int lonesha256 (unsigned char* out, const unsigned char* in, unsigned long inlen) {
    //Writes the sha256 hash of the first "inlen" bytes in buffer "in" to buffer "out".
    //Returns 0 for success, may return non-zero in future versions to indicate error.
    const uint32_t K[64] = {
